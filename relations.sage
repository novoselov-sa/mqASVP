...
def print_mem_usage():
  print(f"Memory usage: {psutil.Process().memory_info().rss / 1024.0^3} GB")
...
# additional characters to get rid of obtructions
USE_SCHIROKAUER_MAPS = False
USE_ADDITIONAL_QUAD_CHARACTERS = False
...
# The list of small field elements is split into two parts in such a way that
# second part contains sigma-conjugates of the first part.
#
# Inputs:
...
#
# Outputs:
# -- list A of base field elements for S-units (first part).
# -- list Ac of sigma-conjugates of field elements from A (second part).
...
# -- matrix Aqc is the same as Aq but for conjugates Ac of A.
# -- list Adenoms of denominators of A.
# -- list qs of primes that were used for characters creation.
@memoized
def get_qstuff_internal(d, high, seed, cheat=False):
  K = field.field(d)
  N = 2^len(d)
  k0 = K(N*[0],1) # TODO: this is 0 in K. Why do we need it???
  ...
  for a in range(len(A)):
    if A[a] == k0:
      ...
      # 
      aq = A[a].get_qs(0,(high + 64), cheat=cheat)
      aqc = Ac[a].get_qs(0,(high + 64), cheat=cheat)

      #assert A[a].denom == Ac[a].denom
      #print('A[a]:', A[a])
...
@nprofile.profile
def get_qstuff(d, high, seed, cheat=False):
	#print('get_qstuff seed:', seed)
  return get_qstuff_internal(d,high,seed,cheat=cheat)
...
#      return hash(tuple(self.factors) + tuple(self.powers) + tuple(self.conj))
    ...
    def symbols_zeta_log(f, Aq, Aqc, Adenoms, qs):
      return char.altfinalzeta(*get_qinfo(f, Aq, Aqc, Adenoms, qs))
...
@nprofile.profile
def get_symbols(d, n, S, seed, cheat=False):
  A, Ac, Aq, Aqc, Adenoms, qs = get_qstuff(d, n, seed, cheat=cheat)
  return matrix(GF(2), [u.symbols_log(Aq, Aqc, Adenoms, qs) for u in S])

@nprofile.profile
def get_symbols_zeta(d, n, S, seed, cheat=False):
  A, Ac, Aq, Aqc, Adenoms, qs = get_qstuff(d, n, seed, cheat=cheat)
  return matrix(Zmod(2^(len(d)-1)), [u.symbols_zeta_log(Aq, Aqc, Adenoms, qs) for u in S])
...
def relations_internal(d, file, seed, food=None):
  ...
  #     S0.append(s)
  #   else:
  #     print(f"{s.generator(A,Ac)}")
  # print(f"-> {len(S) - len(S0)} units filtered")
  # S = S0
  ...
  #   A,Ac = lift_ais(d, seed)
  #   for rel in S:
  #     su = rel.generator(A, Ac)
  #     for i in range(len(rel.factors)):
  #       I = K_sage.ideal([FB[i].prime, K_sage(FB[i].elts)])
  #       assert su.valuation(I) == rel.factors[i], f"Wrong computation of S: {su.valuation(I)} != {rel.factors[i]}"
  ...
  cheat = False
	...
	# it should (?) be of length |factor-base of the field above|
 	#	TODO: EXPLAIN THIS!!!
  A, Ac, Aq, Aqc, Adenoms, qs = get_qstuff(d, len(newv), seed, cheat=cheat)
  ...
  #M = get_symbols(S, Aq, Aqc, Adenoms, qs)
	...
	#
  M = get_symbols(d, len(newv), S, seed, cheat=cheat)
  #print(M)
  
  print(f"-> M dimensions: {M.nrows()} x {M.ncols()}, len(qs) = {len(qs)}")

  if USE_SCHIROKAUER_MAPS and all([mod(di,4) == 1 for di in d]) and len(d) > 2:
    # 2 is unramified in O_K
    ...
    # check that (-1)^e - 1 = 0 mod l^i. So, Schirokauer map will ignore the sign.
    assert mod(schirokauer.exponent(d, 2, len(d)-1), 2) == 0

    for i in range(len(A)):
      A_sch.append(A[i].schirokauer_map(2, len(d)-1))
      Ac_sch.append(Ac[i].schirokauer_map(2, len(d)-1))

    M_sch = []
    for rel in S:
      powers = vector(ZZ, vector(rel.powers) * r)
      conj = vector(ZZ, vector(rel.conj) * r)
      v = [powers[i] * A_sch[i] for i in range(len(powers))]
      v += [conj[i] * Ac_sch[i] for i in range(len(conj))]
      sch = sum(v)
      sch = vector(ZZ, sch) / r
      # since we work with r-powers the sch belongs to {0,1}
      # otherwise, the relation is not an r-power
      assert set(sch).issubset({0,1}), f"{set(sch)}"
      sch = vector(GF(2), sch)
      M_sch.append(sch)
    M_sch = matrix(GF(2), M_sch)
    print(f"[M_sch]\n{M_sch}")
    if M == None:
      M = M_sch
    else:
  ...
  #if len(d) > 2:
  if USE_ADDITIONAL_QUAD_CHARACTERS:
    print("Computing additional quadratic characters")
    sys.stdout.flush()
    high = 16
    A_2 = []
    Ac_2 = []
    ...
    for i in range(len(A)):
      if A[i] == K.zero():
        A_2.append(vector(GF(2), [0]*high))
      else:
        A_2.append(vector(GF(2), A[i].symbols2_log(0, high)))
      if Ac[i] == K.zero():
        Ac_2.append(vector(GF(2), [0]*high))
      else:
        Ac_2.append(vector(GF(2),Ac[i].symbols2_log(0, high)))
    #print(f"[A_2]\n{A_2}")
    #print(f"[Ac_2]\n{Ac_2}")
    M_2 = []
    for rel in S:
      powers = vector(ZZ, vector(rel.powers) * r)
      conj = vector(ZZ, vector(rel.conj) * r)
      v = [powers[i] * A_2[i] for i in range(len(powers))]
      v += [conj[i] * Ac_2[i] for i in range(len(conj))]
      v = sum(v)
    ...
    # we append symbols for -1, so the detection of squares
    # will be performed upto sign.
    v = K.from_ZZ(-1).symbols2_log(0, high)
    M_2 = M_2.stack(matrix(GF(2), v))

    print(f"[M_2]\n{M_2}")
    if M == None:
      M = M_2
    else:
      v = vector(GF(2), [not mod(-1, qs[i]).is_square() for i in range(len(qs))])
      assert set(v) == {0}
      #print(f"v = {v}")
      #M = M.stack(v) # for -1
      M = M.stack(vector(GF(2), [0]*M.ncols())) # for -1
      M = M.augment(M_2)
  ...
  #compute kernel
  e = kernel(n, M)
  #e = pari.matkermod(matrix(ZZ,M).transpose(), 2^(len(d)-1))
  #e = matrix(GF(2), e).transpose()
  #e = matrix(ZZ, e).transpose()
  #print("kernel", e)

  if USE_ADDITIONAL_QUAD_CHARACTERS:
    # getting rid of virtual columns
    e = matrix(GF(2), e)
    e = e[:,:-1] # exclude column corresponding to -1
  ...
  if CHECK_ROOTS:
    ...
    if len(d) <= 3:
      for rel in E:
        gn = rel.generator(A,Ac)
        a = (gn^(1/2))^(2^(len(d)-1))
        sq = False
        try:
          a_ev = a.evaluate()
          b_sage = a_ev.to_sage(K.sage())
          b = b_sage.nth_root(2^(len(d)-1))
        ...
        if not sq:
          try:
            ...
            e2 = e
            print(f" -({e1},{e2}) ", end="", flush=True)
            print(f"\na = {a_ev}")
            print(f"\nrel.a = {(gn).to_sage(K.sage())}")
            print(f"\nrel.factors = {vector(QQ, rel.factors)/2}")
            OK = K.sage().ring_of_integers()
            if b_sage.is_integral():
              print(f"\nis_unit? {OK(b_sage).is_unit()}")
            else:
              print(f"\nis_integral? False")
            print(f"is_square? {b_sage.is_square()}")
            
      print()
  
  # The following code will also filter out sqrts modulo units.
  # print("Filtering out non-squares ...")
  # E0 = []
  # for rel in E:
  #   gn = rel.generator(A,Ac)
  #   sy = gn.altsymbols_log(0, len(newv), cheat=True)
  #   if set(sy) == {0}:
  #     E0.append(rel)
  #   else:
  #     print("non-square was filtered!")
  #     print(f"{rel}")
  # print(f"{len(E) - len(E0)} non-squares was filtered out of {len(E)} elements")
  # E = E0
