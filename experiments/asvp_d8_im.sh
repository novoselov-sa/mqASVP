...
sage trees_generation.sage -3 -7 -11 --gm-add=$gm_add --no-ramified-primes --no-hard > ${fn}_trees.log 2>&1
...
for ((i=1;i<=loops;i++)); do
    # dlog
    fn="experiments/dlog_d${m}_im_gm_add_${gm_add}_no_hard_unramified_quadchars_${i}"
    sage dlog_cyc_rat.sage > ${fn}.log 2>&1
    zip -9 -r ${fn}.zip ${fn}.log experiments/dlogs
    
    # asvp
    fn="experiments/asvp_d${m}_im_gm_add_${gm_add}_no_hard_unramified_quadchars_${i}"
