...
n=5
# clgp
fn="experiments/clgp_d${m}_im_gm${gm_add}_quadchars"
sage trees_generation.sage -3 -7 -11 -19 -23 --gm-add=$gm_add > ${fn}_trees.log 2>&1
...
for ((i=1;i<=loops;i++)); do
    # dlog
    fn="experiments/dlog_d${m}_im_gm${gm_add}_quadchars_dlqc_${i}"
    sage dlog_cyc_rat.sage --quadchars > ${fn}.log 2>&1
    zip -9 -r ${fn}.zip ${fn}.log experiments/dlogs
    
    # asvp
    fn="experiments/asvp_d${m}_im_gm${gm_add}_quadchars_dlqc_${i}"
