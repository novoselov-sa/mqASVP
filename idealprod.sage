...
def idealprods(d, d_parent = (), food = None, FB = None):
  ...
  class IP:
    ...
    def generator(f, SU):
      '''
      Returns a generator of ideal product when this product is principal.
    ...
    def normalize_valuations(f, J, fb = None):
      '''
      Changes exponent vector b in I = alpha prod_i P_i^(b_i) in such a way that v_(P_i)(J) = v_(P_i)(I) for each P_i in the factor base. 
      '''
    ...
    def reduce_norms(f):
      return IP(f.element.reduce_norms(), f.powers)

    def reduce_powers_mod_sunits(f, SU, drift=0):
      drift_v = vector([drift] * len(f.powers))
      A = SU.relations_matrix()
      hpowers = vector(f.powers)
      helement = f.element
      t = walltime()
      w = clgp.approx_CVP(d, list(hpowers + drift_v))
      assert len(w) == len(SU.gens())
      print(f"[reduce_powers_mod_sunits_acvp|{walltime(t)} sec.]", flush=True)
      t = walltime()
      s = prod([SU.gens()[i]^w[i] for i in range(len(w))], KC.one())
      print(f"[reduce_powers_mod_sunits_s|{walltime(t)} sec.]", flush=True)
      t = walltime()
      helement *= s
      hpowers -= vector(w) * A
      print(f"[reduce_powers_mod_sunits_div|{walltime(t)} sec.]", flush=True)
      #assert (vector(f.powers) - vector(w) * A + drift_v).norm(infinity) <= drift, "drift is too small"
      # if (vector(f.powers) - vector(w) * A + drift_v).norm(infinity) <= drift:
      #   print("drift is too small:")
      #   print(f"t = {vector(w) * A}")
      #   print(f"e = {f.powers}")
      #   print(f"|e + s - t|_inf = {(vector(f.powers) - vector(w) * A + drift_v).norm(infinity)}")
    ...
    def short_element(f, SU, drift=None, fprec_increment=None):
      if drift == None:
          drift_v = vector([0] * len(f.powers))
      else:
          drift_v = drift
      A = SU.relations_matrix()
      hpowers = -vector(f.powers)
      ...
      while not done:
          ...
          hpowers -= vector(w) * A
          #print(f"[reduce_powers_mod_sunits_div|{walltime(t)} sec.]", flush=True)
          bound = RR(sqrt(2*log(8*A.ncols())) * 2*sqrt(2^len(d)))
          if hpowers.norm(infinity) > bound:
              print("Warning! The vector returned by Approx-CVP solver is too long. Try to increase FPLLL precsion.")
              print(f"|a-b-drift|_inf = {hpowers.norm(infinity)}, len(FB) = {A.ncols()}, bound = {bound}")
              if fprec_increment == None:
                  done = True
              else:    
                  FPLLL.set_precision(FPLLL.get_precision()+fprec_increment)
              print(f"new precision: {FPLLL.get_precision()}")
          else:
              done = True
      return IP(helement, hpowers)
    ...
    def sqrt_rat(f, SU, quadchars=False):
      return ideal_sqrt_cyc.ideal_sqrt_rat(d, f.element, f.powers, SU, quadchars=quadchars)
