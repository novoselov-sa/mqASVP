...
USE_EXCEPTIONAL_PRIMES = True # include primes p s.t. p | [O_K : Z[theta_K]] where is s.t. K = Q(theta_K)
USE_SPLIT_PRIMES_ONLY = False # include only primes that split completely
...
@nprofile.profile
def build_tree(K, tree, norm_bound=None, primes_bound=None, aut=None, aut2=None, parent_id=None, primes=None, append_primes=[]):
    K_id = K.id(parent_id=parent_id)
    d_K = K.getd()
        ...
        while p <= primes_bound:
            ...
            for di in d_K:
                if not is_square(Mod(di, p)):
                    sq = False
                    break
            if (sq and USE_SPLIT_PRIMES_ONLY) or (not USE_SPLIT_PRIMES_ONLY):
                primes.append(p)
    ...
    if len(d_K) == 1:
        K2 = NumberField([x^2-d_K[0]], names=K.names()[0])
        ...
        for p in primes:
            ...
            for i,el in enumerate(F):
                ...
                if True: #J.absolute_norm() <= norm_bound or alpha_K2.is_rational():
                    ...
                    print("(excluded ideal) I = ({},{}), N(I)={}".format(p, alpha.to_sage(names=K.names()), J.absolute_norm()))
            p = next_prime(p)
        return tree
    ...
    K_s_names = K.names()[:-1]
    K_s = field.field(d_s)
    K_s.set_names(names=K_s_names)
    K_s_id = K_s.id(parent_id=K_id)
    #
    d_t = d_K[:-2] + (d_K[-1],)
    K_t_names = K.names()[:-2] + (K.names()[-1],)
    K_t = field.field(d_t)
    K_t.set_names(names=K_t_names)
    K_t_id = K_t.id(parent_id=K_id)
    #
    d_st = d_K[:-2] + (d_K[-1] * d_K[-2],)
    K_st_names = K.names()[:-2] + (K.names()[-2] + K.names()[-1],)
    K_st = field.field(d_st)
    K_st.set_names(names=K_st_names)
    K_st_id = K_st.id(parent_id=K_id)
    #
    tree[K_id]['subfields'] = [K_s_id, K_t_id, K_st_id]

    build_tree(K_t, tree, primes_bound=primes_bound, norm_bound=norm_bound,
                          parent_id=K_id,
                          aut=t,
                          primes=primes)
    build_tree(K_s, tree, primes_bound=primes_bound, norm_bound=norm_bound,
                          parent_id=K_id,
                          aut=s,
                          primes=primes)
    aut2 = [0] * (len(d_K) - 2)  + [1] # aut2 = res_{K/K_st}(s)
    build_tree(K_st, tree, primes_bound=primes_bound, norm_bound=norm_bound,
                           parent_id=K_id,
                           aut=st, aut2=aut2,
                           primes=primes)
        ...
        for i,alpha_t in enumerate(data['elements']):
            ...
            for J,e in I_up:
                ...
                if J[1] in elts:
                    ...
                    K_sage = NumberField([x^2 - di for di in d_K], names=K.names())
                    i = find_prime_ideal(K_sage, p, J[1], elts, K.names())
                    if i != None:
                        vec[i] = e
                        found = True
                if not found:
    ...
    for p, data in K_st_primes.items():
        ...
        for i,alpha_st in enumerate(data['elements']):
            ...
            if VERIFY:
                ...
                K2 = NumberField([x^2-di for di in d_K], names=K.names())
                I_st_K2 = K2.ideal(p, K2(str(K(alpha_st).to_sage(names=K.names()))))
                I_st_K2_aut = K2.ideal(p, K2(str(K(alpha_st.apply_aut(aut2)).to_sage(names=K.names()))))
                if len(ids2) == 2:
                    J1,J2 = ids2
                    J1_K2 = K2.ideal(p, K2(str(J1[0][1].to_sage(names=K.names())))) ^ J1[1]
                    J2_K2 = K2.ideal(p, K2(str(J2[0][1].to_sage(names=K.names())))) ^ J2[1]
                elif len(ids2) == 1:
                    J1 = ids2[0]
                    J1_K2 = K2.ideal(p, K2(str(J1[0][1].to_sage(names=K.names())))) ^ J1[1]
                    J2_K2 = K2.ideal(1)
            ...
            for I,e in I_up:
                #print(f"I[1] = {I[1].to_sage(names=K.names())}, e = {e}")
                ...
                if alpha in elts:
                    ...
                    K_sage = NumberField([x^2 - di for di in d_K], names=K.names())
                    i = find_prime_ideal(K_sage, p, alpha, elts, K.names())
                    if i != None:
                        vec[i] = e
                        found = True
                if not found:
...
def save_tree(tree, K, folder = "trees/"):
    files = conv_tree_to_BV_format(tree, K.id(), names=K.names())
...
def save_field(d, folder = "trees/"):
    open(f"{folder}/field.txt", "w").write(str(d))

def get_food(folder = "trees/"):
    fn = f"{folder}/food.txt"
...
def get_d(folder = "trees/"):
    fn = f"{folder}/field.txt"
    if os.path.exists(fn):
        data = open(fn).read()
        field = eval(preparse(data))
    else:
        food = get_food()
        field = tuple(sorted(food.values(), key=lambda di: abs(di)))
    return field

def get_field(folder = "trees/"):
    K = field.field(get_d(folder))
    K.set_names(get_food(folder))
    return K
